...
*/
          ...
          function startsWith(str, searchString, position) {
            return (
              str.substr(position || 0, searchString.length) === searchString
            )
          }
        ...
        function (require, module, exports) {
          ...
          function formatAttributes(attributes) {
            return attributes.map(function (attribute) {
              ...
              var key = parts[0]
              var value =
                typeof parts[1] === 'string' ? unquote(parts[1]) : null
              return {
                key: key,
                value: value,
              }
          ...
          function lexTagAttributes(state) {
            ...
            for (var i = 0; i < wLen; i++) {
              ...
              if ((0, _compat.endsWith)(word, '=')) {
                ...
                if (
                  _secondWord &&
                  !(0, _compat.stringIncludes)(_secondWord, '=')
                ) {
        ...
        function (require, module, exports) {
          ...
          function parse(state) {
            ...
            while (cursor < len) {
              ...
              var isClosingTag = (0, _compat.arrayIncludes)(
                options.closingTags,
                tagName,
              )
              ...
              if (shouldRewindToAutoClose) {
                ...
                shouldRewindToAutoClose = !hasTerminalParent(
                  tagName,
                  stack,
                  terminals,
                )
              ...
              var hasChildren = !(
                attrToken.close ||
                (0, _compat.arrayIncludes)(options.voidTags, tagName)
              )
              if (hasChildren) {
                var size = stack.push({
                  tagName: tagName,
          ...
          function toHTML(tree, options) {
            return tree
              .map(function (node) {
                ...
                var isSelfClosing = (0, _compat.arrayIncludes)(
                  options.voidTags,
                  tagName.toLowerCase(),
                )
                return isSelfClosing
                  ? '<'
                      .concat(tagName)
                      .concat(formatAttributes(attributes), '>')
                  : '<'
                      .concat(tagName)
                      .concat(formatAttributes(attributes), '>')
                      .concat(toHTML(children, options), '</')
                      .concat(tagName, '>')
              })
              .join('')
          }
